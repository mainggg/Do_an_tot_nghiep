  ...
  limit(decimals: number): NumberBuilder {
    ...
    if (decimalPart && decimalPart.length > decimals) {
      return new NumberBuilder(
        parseFloat(`${firstPart}.${decimalPart.substr(0, decimals)}`)
      );
    }
...
export class NumberInputDirective implements ControlValueAccessor, OnChanges {
  ...
  private onChange: (value?: any) => any = noop;
  ...
  @HostListener('input', ['$event']) public onInput(event: Event): void {
    ...
    if (!this.isValidInput(value)) {
      ...
      const parsedNumber = numeral(value).value();
  ...
  private verifySettings(): void {
    if (this.min > this.max) {
      throw new Error(
        'The max modelValue should be bigger than the minInput modelValue'
      );
    }
  ...
  //       ? String(this.modelValue).replace('.', decimal)
  //       : numeral(this.modelValue).format(this.format);
  //   }
  ...
  private renderDisplayValue(): void {
    ...
    if (!isNil(this.modelValue)) {
      const stringValue = String(this.modelValue);
      const hasDecimal = stringValue.includes('.');
      const { decimal, thousands } = numeral.localeData().delimiters;

      if (this.focused) {
        // Loại bỏ dấu ',' khi focus vào ô input
        this.displayValue = stringValue.replace(
          new RegExp(`\\${thousands}`, 'g'),
          ''
        );
      } else {
        if (hasDecimal) {
          ...
          if (decimalPart.length > 0 && decimalPart.length <= 4) {
            this.displayValue = `${numeral(integerPart).format(
              '0,0'
            )}${decimal}${decimalPart}`;
          } else {
            this.displayValue = `${numeral(integerPart).format(
              '0,0'
            )}${decimal}${decimalPart.substr(0, 4)}`;
          }
        } else {
          this.displayValue = numeral(stringValue).format('0,0');
        }
      }
    }
